...
You can navigate programmatically using the `useNavigate` hook. This is useful for situations like redirecting after a form submission.
...
You can nest routes inside other routes. This is useful for creating layouts where a part of the UI is shared across multiple pages, like a dashboard.

<!-- element class="fragment" -->

The parent route component renders an `<Outlet>` component, which then renders the matched child route component.
...
Sometimes you need to create routes for a pattern, like a user profile page. You can use dynamic segments in the URL, prefixed with a colon `:`.
...
You can then access the dynamic segment (the `userId` in this case) from the component using the `useParams` hook.
