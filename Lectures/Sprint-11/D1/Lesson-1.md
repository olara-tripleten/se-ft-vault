...
In a single-page application (SPA), you don't have separate HTML files for each page. Instead, you have one HTML file and use JavaScript to dynamically change the content.

<!-- element class="fragment" -->

A router helps manage what content is displayed to the user based on the URL, creating the illusion of multiple pages.
...
Then, you wrap your main application component with `<BrowserRouter>`.
<!-- element class="fragment" -->
...
Instead of using `<a>` tags, React Router uses `<Link>` components to prevent the page from reloading.

<!-- element class="fragment" -->

Here's a basic example of setting up navigation:
...
You can create a "Not Found" page for any routes that don't match by using a `path` of `*`.
