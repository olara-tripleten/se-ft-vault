...
When rendering a list of components, you should provide a unique `key` prop to each item. This helps React identify which items have changed, are added, or are removed.
...
In React, form elements like `<input>`, `<textarea>`, and `<select>` maintain their own state. To make them controlled components, you can hold their value in the component's state.
...
A higher-order component is a function that takes a component and returns a new component. They are used for reusing component logic.
