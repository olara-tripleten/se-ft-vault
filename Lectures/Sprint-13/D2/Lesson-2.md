...
Let's practice validating data on the server-side.

<!-- element class="fragment" -->

We'll build an Express endpoint that uses regular expressions to check if an incoming `email` and `password` are valid.

---

## Express Server Setup

First, we need a basic Express server. Make sure you have `express` installed (`npm install express`).
...
A simple, common regex for validating an email format.
...
A regex for enforcing a strong password policy.
...
Now, let's create a `/register` endpoint and place our validation logic directly inside it.
...
- Server-side validation is critical for security and data integrity.
- Regular expressions are a powerful tool for defining and enforcing complex input rules.
- Always validate any data that comes from an external source!
